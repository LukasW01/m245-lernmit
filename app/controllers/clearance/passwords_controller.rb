...
module Clearance
  class PasswordsController < Clearance::BaseController
    before_action :ensure_existing_user, only: %i[edit update]
    before_action :ensure_email_present, only: [:create]
    skip_before_action :require_login, only: %i[create edit new update], raise: false
    ...
    def ensure_email_present
      return unless email_from_password_params.blank?
    ...
    def ensure_existing_user
      return if find_user_by_id_and_confirmation_token
    ...
    def flash_failure_when_forbidden
      flash.now[:alert] = translate(:forbidden,
                                    scope: %i[clearance controllers passwords],
